...
def main():
    ...
    print(f"[DEBUG] 기존 row_hash 수: {len(existing_row_hashes)}")

    print("[STEP] 처리 완료 파일키 조회 시작")
    processed_file_keys = sheet_client.get_processed_file_keys()
    print(f"[DEBUG] 처리 완료 파일키 수: {len(processed_file_keys)}")
    ...
    for file_meta in files:
        ...
        try:
            ...
            print(f"[DEBUG] 파일 길이(char): {len(text)}")

            print(f"[STEP] 파싱 시작: {file_name}")
            parsed_rows = parse_chat_text(text)
            print(f"[DEBUG] 파싱 완료: {len(parsed_rows)} rows")
            ...
            print(f"[DEBUG] 업로드 대상 row 수: {len(upload_rows)}")
